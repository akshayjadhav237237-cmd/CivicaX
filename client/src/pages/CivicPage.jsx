    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto w-full">
      ...
      <GlassCard padding="p-4" className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select className="bg-white/50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none">
      ...
      {/* Reports Grid */}
      {isLoading ? (
        <div className="text-center py-20 text-slate-500 dark:text-slate-400">Loading reports...</div>
      ) : reports.length === 0 ? (
        <GlassCard className="text-center py-16">
          <CheckCircle size={48} className="mx-auto text-green-500 mb-4 opacity-50" />
          <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300">No issues reported</h3>
          <p className="text-slate-500 dark:text-slate-400 mt-2">Your community infrastructure looks good.</p>
        </GlassCard>
      ) : (
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="w-full md:w-1/2 flex flex-col gap-4">
              {selectedReport.imageUrl ? (
                <img src={`http://localhost:3001${selectedReport.imageUrl}`} alt="Evidence" className="w-full h-48 object-cover rounded-xl border border-slate-200 dark:border-slate-800" />
              ) : (
