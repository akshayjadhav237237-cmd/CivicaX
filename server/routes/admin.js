...
/**
 * GET /api/v1/admin/smap-test
 * Manually trigger and test the NASA SMAP soil moisture fetcher
 */
router.get('/smap-test', adminGuard, async (req, res) => {
  try {
    const { fetchSMAPSoil } = require('../modules/satellite/smapSoil');
    const result = await fetchSMAPSoil();
    res.json({ success: true, data: result });
  } catch (err) {
    logger.error('SMAP test route error:', err);
