...
// Startup check
const startupToken = process.env.NASA_EARTHDATA_TOKEN;
if (!startupToken) {
  logger.warn('[SMAP] ⚠️ NASA Earthdata token is missing! SMAP L3 Soil Moisture will use the 0.35 fallback with source: "fallback_no_auth"');
} else {
  logger.info('[SMAP] NASA Earthdata token is present. Real SMAP satellite data fetches will be attempted.');
}
...
async function fetchSMAPValueFromOpendap(opendapBaseUrl, granuleFilename, token) {
  ...
  try {
    ...
    logger.warn(`[SMAP] Primary OPeNDAP request failed: ${err.message}. Trying alternative NSIDC endpoint...`);
    const altBaseUrl = `https://n5eil01u.ecs.nsidc.org/SMAP/SPL3SMP_E.006/${granuleFilename}`;
    url = `${altBaseUrl}.ascii?Soil_Moisture_Retrieval_Data_AM/soil_moisture%5B${KEDARNATH_ROW}%5D%5B${KEDARNATH_COL}%5D`;
    logger.info(`[SMAP] Fetching from alternative NSIDC OPeNDAP: ${url}`);
    ...
    if (!res.ok) {
      throw new Error(`Alternative OPeNDAP HTTP ${res.status}: ${res.statusText}`);
    }
  }
